...
export function DroneDetectorDetailClient({ detector, similar }: Props) {
  const [currentImageIndex, setCurrentImageIndex] = useState(0);
  const gallery = detector.gallery ?? [];
        ...
        <div className="grid grid-cols-1 lg:grid-cols-2 gap-12 mb-12">
              ...
              {currentImage ? (
                ...
                <div className="absolute inset-0 flex items-center justify-center">
                  <div className="w-64 h-64 border border-white/10 rounded-full flex items-center justify-center">
                    <span className="text-6xl font-stencil text-white/10">ДД</span>
                  </div>
                </div>
              )}
            </div>
                ...
                {gallery.map((img: string, index: number) => (
                  ...
                  >
                    <div className="relative w-full h-full">
                      <Image
                        src={img}
                        alt={`${detector.model} - вид ${index + 1}`}
                        fill
          ...
          <div>
            ...
            {detector.description && (
              <div
                className="text-gray-300 text-lg leading-relaxed mb-8 border-l-2 border-aero-accent pl-4"
                dangerouslySetInnerHTML={{ __html: detector.description }}
              />
            )}
        ...
        {detector.detailedInfo && (
          <div className="bg-black/50 border border-white/10 tactical-clip p-8 mb-12">
            ...
            <div
              ...
              dangerouslySetInnerHTML={{ __html: detector.detailedInfo }}
            />
          </div>
        )}
        ...
        {similar.length > 0 && (
          <div className="border-t border-white/10 pt-12">
            ...
            <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-6">
              {similar.map((item) => (
                <DetectorCard key={item.id} detector={item} />
              ))}
            </div>
          </div>
        )}
